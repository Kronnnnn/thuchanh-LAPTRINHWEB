...
<body>
    ...
    <div class="container my-3">
        <h2 class="py-2">Kết quả tìm kiếm cho <em>"
        ...
        <div class="row">
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ?>
                <script> document.getElementById("cat").innerHTML = "Danh mục: ";</script>
                <?php
                $noResult = false;
                ...
                echo '<div class="col-xs-3 col-sm-3 col-md-3">
                    <div class="card" style="width: 18rem;">
                        ...
                        <div class="card-body">
                            <h5 class="card-title"><a href="viewQAList.php?catid=' . $catId . '">' . $catname . '</a></h5>
                            <p class="card-text">' . substr($catdesc, 0, 29) . '...</p>
                            <a href="viewQAList.php?catid=' . $catId . '" class="btn btn-primary">View All</a>
                        </div>
                    </div>
                </div>';
            }
    ...
    <div class="container my-3" id="cont">
        ...
        <div class="row">
            <?php
            $query = $_GET["search"];
            $sql = "SELECT * FROM `flower` WHERE MATCH(pizzaName, pizzaDesc) against('$query')";
            $result = mysqli_query($conn, $sql);
            while ($row = mysqli_fetch_assoc($result)) {
                ?>
                <script> document.getElementById("iteam").innerHTML = "Sản phẩm: ";</script>
                <?php
                $noResult = false;
                $pizzaId = $row['pizzaId'];
                ...
                echo '</form>
                                <a href="viewQuanAo.php?pizzaid=' . $pizzaId . '"><button class="btn btn-primary">Quick View</button></a>
                            </div>
                        </div>
                    </div>
                </div>';
